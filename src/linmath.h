...
#define LINMATH_H_DEFINE_VEC(n)                                                 \
	...
	LINMATH_H_FUNC void vec##n##_set(vec##n r, vec##n const v)                  \
	{                                                                           \
		int i;                                                                  \
		for (i = 0; i < n; ++i)                                                 \
			r[i] = v[i];                                                        \
...
LINMATH_H_FUNC void vec3_rotate_around(vec3 r, const vec3 around, float angle)
{
	...
	double Q[3][3];
	Q[0][0] = around[0] * around[0] * C + c;
	Q[0][1] = around[1] * around[0] * C + around[2] * s;
	Q[0][2] = around[2] * around[0] * C - around[1] * s;

	Q[1][0] = around[1] * around[0] * C - around[2] * s;
	Q[1][1] = around[1] * around[1] * C + c;
	Q[1][2] = around[2] * around[1] * C + around[0] * s;

	Q[2][0] = around[0] * around[2] * C + around[1] * s;
	Q[2][1] = around[2] * around[1] * C - around[0] * s;
	Q[2][2] = around[2] * around[2] * C + c;

	result[0] = (r[0] * Q[0][0] + r[1] * Q[1][0] + r[2] * Q[2][0]);
	result[1] = (r[0] * Q[0][1] + r[1] * Q[1][1] + r[2] * Q[2][1]);
	result[2] = (r[0] * Q[0][2] + r[1] * Q[1][2] + r[2] * Q[2][2]);
	r[0] = result[0];
	r[1] = result[1];
	r[2] = result[2];
}

LINMATH_H_FUNC void mat4x4_rotate_around_axis(mat4x4 r, vec3 const axis, float angle)
{
	...
	r[0][0] = cosO + ux * ux * (1 - cosO);
	r[1][0] = ux * uy * (1 - cosO) - uz * sinO;
	r[2][0] = ux * uz * (1 - cosO) + uy * sinO;
	r[3][0] = 0;

	r[0][1] = ux * uy * (1 - cosO) + uz * sinO;
	r[1][1] = cosO + uy * uy * (1 - cosO);
	r[2][1] = uy * uz * (1 - cosO) - ux * sinO;
	r[3][1] = 0;

	r[0][2] = uz * ux * (1 - cosO) - uy * sinO;
	r[1][2] = uz * uy * (1 - cosO) + ux * sinO;
	r[2][2] = cosO + uz * uz * (1 - cosO);
	r[3][2] = 0;
...
LINMATH_H_FUNC void mat4x4_rotate_around_coordsys(
	mat4x4 r,
	vec3 const origin,
	vec3 const dir_x,
	vec3 const dir_y,
...
{
	vec3 origin_to_zero;
	vec3_scale(origin_to_zero, origin, -1.0f);
	mat4x4 translate_to_zero;
	mat4x4 rotation_around_x;
	mat4x4 rotation_around_y;
	mat4x4 rotation_around_z;
	mat4x4 translate_to_origin;

	mat4x4_translate(translate_to_zero, origin_to_zero[0], origin_to_zero[1], origin_to_zero[2]);
	mat4x4_translate(translate_to_origin, origin[0], origin[1], origin[2]);

	mat4x4_rotate_around_axis(rotation_around_x, dir_x, angle_x);
	mat4x4_rotate_around_axis(rotation_around_y, dir_y, angle_y);
	mat4x4_rotate_around_axis(rotation_around_z, dir_z, angle_z);

	mat4x4_identity(r);
	mat4x4_mul(r, translate_to_zero, r);
	mat4x4_mul(r, rotation_around_x, r);
	mat4x4_mul(r, rotation_around_y, r);
	mat4x4_mul(r, rotation_around_z, r);
	mat4x4_mul(r, translate_to_origin, r);
}
